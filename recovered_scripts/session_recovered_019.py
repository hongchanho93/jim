...
def nearest_b_for_a(a: pd.Timestamp, b_dates: List[pd.Timestamp]) -> pd.Timestamp:
    # choose closest by absolute calendar-day distance, tie -> earlier date
...
def main() -> None:
    ...
    # Build matched signals but signal day = B (bull alignment day)
    matched_signals_bday: List[Tuple[str, pd.Timestamp, pd.Timestamp]] = []  # (code, a_date, b_date)
