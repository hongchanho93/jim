...
def pick_daily_15m_files(base: Path) -> Tuple[Path, Path]:
    daily_file = None
    m15_file = None
...
def turnover_condition_ok(month_turn_map: Dict[int, float], signal_date: pd.Timestamp) -> bool:
    ...
    # exclude current month: [sig_ord-18, sig_ord-1]
...
def main() -> None:
    base = Path('.')
    daily_file, m15_file = pick_daily_15m_files(base)
    print('USING_DAILY', daily_file.name)
    print('USING_15M', m15_file.name)

    daily = pd.read_parquet(daily_file, columns=['stock_code', 'date', 'open', 'high', 'low', 'close', 'turnover'])
    ...
    matched_signals_bday: List[Tuple[str, pd.Timestamp, pd.Timestamp]] = []
    ...
    for idx, code in enumerate(candidate_codes, start=1):
        ...
        for a in s1_dates:
            ...
            if hits:
                ...
                matched_signals_bday.append((code, a, b))

        if idx % 100 == 0 or idx == len(candidate_codes):
            print(f'PROGRESS_MATCH {idx}/{len(candidate_codes)} matched={len(matched_signals_bday)}')

    print('MATCHED_SIGNAL_COUNT_A_BASE', len(matched_signals_bday))
    ...
    eligible_returns: Dict[int, List[float]] = {h: [] for h in horizons}
    eligible_count = 0
    passed_turnover_count = 0

    for code, a_date, b_date in matched_signals_bday:
        g = by_code.get(code)
        if g is None or g.empty:
            continue

        # new filter: at least one month turnover >100% in previous 18 complete months before B-date
        if not turnover_condition_ok(month_turn_map_by_code[code], b_date):
            continue
        passed_turnover_count += 1

        trade_dates = g['date'].to_numpy(dtype='datetime64[ns]')
        close = g['close'].to_numpy(dtype=float)
        high = g['high'].to_numpy(dtype=float)

        idx = map_signal_to_trade_idx(trade_dates, b_date)
        if idx < 0:
            continue
        if idx + 60 >= len(g):
            continue
        ...
        eligible_count += 1
        for h in horizons:
            future_high = float(np.max(high[idx + 1 : idx + h + 1]))
            ret = future_high / base - 1.0
            eligible_returns[h].append(ret)

    print('PASSED_TURNOVER_FILTER', passed_turnover_count)
    print('ELIGIBLE_SIGNALS_60D', eligible_count)

    for h in horizons:
        arr = np.array(eligible_returns[h], dtype=float)
        print(f'H{h}_N', arr.size)
        for th in thresholds:
            hit = int(np.sum(arr >= th))
            pct = (hit / arr.size * 100.0) if arr.size else 0.0
            print(f'H{h}_GE_{int(th*100)}', hit, f'{pct:.2f}%')
