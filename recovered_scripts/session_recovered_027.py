...
def stage2_signal_dates_from_15m(df: pd.DataFrame, start_date: pd.Timestamp, end_date: pd.Timestamp) -> List[pd.Timestamp]:
    if df.empty:
        return []
    data = df.copy()
    ...
    merged = merged[(merged['ts'] >= start_date) & (merged['ts'] <= (end_date + pd.Timedelta(days=1)))].copy()
    if merged.empty:
        return []
...
def load_15m_stock(data15: Path, code: str, start_str: str, end_str: str) -> pd.DataFrame:
    table = pq.read_table(
        data15,
        columns=['stock_code', 'stock_name', 'date', 'time', 'open', 'high', 'low', 'close'],
        filters=[('stock_code', '=', code), ('date', '>=', start_str), ('date', '<=', end_str)],
    )
    if table.num_rows == 0:
        return pd.DataFrame()
...
def main() -> None:
    ...
    recent_start = latest_date - pd.DateOffset(months=2)
    ...
    for code, g in daily.groupby('code', sort=False):
        ...
        s1_dates = [d for d in stage1_signal_dates(frame, ma_window) if lookaround_start <= d <= latest_date]
        if s1_dates:
            stage1_map[code] = s1_dates

    candidate_codes = sorted(stage1_map.keys())
    ...
    for code in candidate_codes:
        s1_dates = stage1_map.get(code, [])
        if not s1_dates:
            continue
        m15_df = load_15m_stock(
            m15_file,
            code,
            (lookaround_start - pd.DateOffset(months=3)).date().isoformat(),
            latest_date.date().isoformat(),
        )
        if m15_df.empty:
            continue
        s2_dates = stage2_signal_dates_from_15m(m15_df, lookaround_start - pd.DateOffset(months=3), latest_date)
        if not s2_dates:
            continue
        ...
        for a in s1_dates:
            ...
            if not (recent_start <= b <= latest_date):
                continue
            if not turnover_condition_ok(month_turn_map_by_code[code], b):
                continue
            rows.append(
                {
                    'stock_code': code,
                    'stock_name': name_map.get(code, ''),
                    'signal_date': b.date().isoformat(),
    ...
    if out.empty:
        print('LATEST_DATE', latest_date.date().isoformat())
        print('RECENT_START', recent_start.date().isoformat())
        print('COUNT', 0)
        return

    out = out.sort_values(['signal_date', 'stock_code']).reset_index(drop=True)
    out = out.drop_duplicates(subset=['stock_code', 'signal_date', 'a_date'])

    print('LATEST_DATE', latest_date.date().isoformat())
    print('RECENT_START', recent_start.date().isoformat())
    print('COUNT', len(out))
    print('UNIQUE_STOCKS', out['stock_code'].nunique())
    print('DATA_BEGIN')
