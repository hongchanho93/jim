...
def count_dedup_signals_since(frame, ma_window):
    frame = frame.copy()
    ...
    close_zone()
    cnt = sum(1 for idx in signal_indices if pd.Timestamp(dates[idx]) >= START_DATE)
    return cnt


def pick_daily_parquet():
    candidates = []
    for p in Path('.').glob('*.parquet'):
        ...
        if {'stock_code', 'date', 'open', 'high', 'low', 'close'}.issubset(cols) and 'time' not in cols:
            candidates.append(p)
    if not candidates:
        raise RuntimeError('No daily parquet found')
    candidates.sort(key=lambda x: x.stat().st_size, reverse=True)
    return candidates[0]

p = pick_daily_parquet()
print('USING_FILE', p.name)

df = pd.read_parquet(p, columns=['stock_code','date','open','high','low','close'])
...
for code, g in df.groupby('code', sort=False):
    ...
    cnt = count_dedup_signals_since(frame, ma_window)
    if cnt > 0:
        stock_with_signal += 1
        total_signals += cnt
        if use_monthly:
