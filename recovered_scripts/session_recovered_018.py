...
def map_signal_to_trade_idx(trade_dates: np.ndarray, signal_date: pd.Timestamp) -> int:
    # backward asof: last trade day <= signal_date
    target = np.datetime64(signal_date)
    pos = int(np.searchsorted(trade_dates, target, side='right')) - 1
    return pos
...
def main() -> None:
    ...
    # Build final matched 2025 signals (A-side dates)
    matched_signals: List[Tuple[str, pd.Timestamp]] = []
    for idx, code in enumerate(candidate_codes, start=1):
        ...
        for a in s1_dates:
            if any(within_two_months(a, b) for b in s2_dates):
                matched_signals.append((code, a))

        if idx % 100 == 0 or idx == len(candidate_codes):
            print(f'PROGRESS_MATCH {idx}/{len(candidate_codes)} matched={len(matched_signals)}')

    print('MATCHED_SIGNAL_COUNT', len(matched_signals))
    ...
    for code, signal_date in matched_signals:
        g = by_code.get(code)
        ...
        idx = map_signal_to_trade_idx(trade_dates, signal_date)
        if idx < 0:
            continue
        # user requirement: must have at least 60 trading days after signal
        if idx + 60 >= len(g):
            continue
