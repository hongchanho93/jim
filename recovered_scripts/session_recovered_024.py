...
def main() -> None:
    ...
    # A-B match and choose nearest B as signal day
    matched_rows = []
    ...
    # apply turnover + 60d eligibility; compute returns per signal row
    eligible = []
    for row in matched_df.itertuples(index=False):
        ...
        rec = {
            ...
            'ret20': float(np.max(high[idx + 1: idx + 21]) / base - 1.0),
            'ret40': float(np.max(high[idx + 1: idx + 41]) / base - 1.0),
            'ret60': float(np.max(high[idx + 1: idx + 61]) / base - 1.0),
        }
        eligible.append(rec)

    eligible_df = pd.DataFrame(eligible)
    if eligible_df.empty:
        raise RuntimeError('No eligible rows after filters')

    print('ELIGIBLE_SIGNAL_ROWS', len(eligible_df))

    # file 1: threshold lists
    detail_rows = []
    summary_rows = []
    for h in HORIZONS:
        col = f'ret{h}'
    ...
    # file 2: code + name + signal_date only
    ...
    file1 = out_dir / '2025阈值清单.xlsx'
    file2 = out_dir / '2025信号清单_代码名称日期.xlsx'

    with pd.ExcelWriter(file1, engine='openpyxl') as w:
        summary_df.to_excel(w, sheet_name='统计汇总', index=False)
        detail_df.to_excel(w, sheet_name='股票清单', index=False)

    with pd.ExcelWriter(file2, engine='openpyxl') as w:
        signal_simple.to_excel(w, sheet_name='信号清单', index=False)
