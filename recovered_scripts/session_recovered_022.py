...
def main() -> None:
    ...
    code_sets: Dict[Tuple[int, int], set] = {(h, int(t*100)): set() for h in horizons for t in thresholds}
    eligible_rows = 0
    ...
    for code, a_date, b_date in matched_signals_bday:
        ...
        for h in horizons:
            ret = float(np.max(high[idx + 1 : idx + h + 1])) / base - 1.0
            for t in thresholds:
                if ret >= t:
                    code_sets[(h, int(t*100))].add(code)

    print('ELIGIBLE_ROWS', eligible_rows)
    for h in horizons:
        for t in [20, 30, 40, 50]:
            codes = sorted(code_sets[(h, t)])
            print(f'LIST_H{h}_GE_{t}_COUNT', len(codes))
            print(f'LIST_H{h}_GE_{t}', ';'.join(codes))
